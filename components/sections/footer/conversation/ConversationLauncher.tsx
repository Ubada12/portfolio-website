...
/**
 * ConversationLauncher Component
 *
 * Full-screen modal that presents conversation-start options
 * (send message, email, or just explore).
 * Triggered from the footer CTA. Includes backdrop blur, glassmorphism
 * card, noise texture overlay, and staggered action buttons.
 */

import { motion, AnimatePresence } from "framer-motion"
import { X } from "lucide-react"
import { getConversationActions } from "./ConversationActions"
import { CONVERSATION_MODAL } from "@/constants/footer"
                ...
                <div className="relative flex items-start justify-between px-6 py-5 border-b border-border/40">
                  <div>
                    <h3 className="text-base font-semibold text-foreground">
                      {CONVERSATION_MODAL.title}
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      {CONVERSATION_MODAL.subtitle}
                    </p>
                  </div>

                  <button
                    onClick={onClose}
                    aria-label="Close conversation launcher"
                    className="
                      rounded-full p-2
