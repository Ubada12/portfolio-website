...
/**
 * EngineeringProcess Component
 *
 * A horizontally-scrolling "sticky card stage" that walks the viewer
 * through the engineering workflow (Discovery → Design → Development →
 * Launch). The section is 400 vh tall; a sticky inner container
 * translates cards left as the user scrolls.
 *
 * Sub-components: ProcessHeader (top bar), CardStage (card strip),
 * ProgressIndicators (bottom dots), GridBackground, FloatingParticles.
 *
 * @performance
 * - Scroll-driven transforms via `useScroll` / `useTransform` (no JS loop)
 * - Particles are SSR-safe (mounted flag) and positions are memoized
 */
...
export const EngineeringProcess = memo(function EngineeringProcess() {
